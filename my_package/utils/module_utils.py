...
class ConvertMode(Enum):
    ...
    NONE = "none"
    # Convert the OmegaConf config to primitive container,
    # Structured Configs are preserved
    PARTIAL = "partial"
    # Fully convert the OmegaConf config to primitive containers
    # (dict, list and primitives).
    ALL = "all"
        ...
        try:
            ...
            if isinstance(obj, ModuleType):
                ...
                try:
                    ...
                    raise ImportError(
                        f"Error loading '{path}':\n{repr(exc_import)}"
                        f"\nAre you sure that '{part}' is importable"
                        f"from module '{parent_dotpath}'?"
                    ) from exc_import
                except Exception as exc_import:
                    raise ImportError(
                        f"Error loading '{path}':\n{repr(exc_import)}"
...
def _extract_pos_args(input_args: Any, kwargs: Any) -> Tuple[Any, Any]:
    ...
    if isinstance(config_args, Sequence):
        ...
        raise ValueError(
            f"Unsupported _args_ type: '{type(config_args).__name__}'."
            f" value: '{config_args}'"
        )
...
def _call_target(
    ...
    try:
        ...
        msg = (
            f"Error in collecting args and kwargs for"
            f" '{_convert_target_to_string(_target_)}':"
            f"\n{repr(e)}"
        )
        if full_key:
            msg += f"\nfull_key: {full_key}"
    ...
    if _partial_:
        try:
            ...
            msg = (
                f"Error in creating partial"
                f"({_convert_target_to_string(_target_)}, ...) object:"
                f"\n{repr(e)}"
            )
            if full_key:
                msg += f"\nfull_key: {full_key}"
...
def _resolve_target(
    ...
    if not callable(target):
        msg = (
            f"Expected a callable target,"
            f" got '{target}' of type '{type(target).__name__}'"
        )
        if full_key:
            msg += f"\nfull_key: {full_key}"
...
def instantiate(config: Any, *args: Any, **kwargs: Any) -> Any:
    """
    Args:
        config (Any): DictConfig by OmegaConf.load(${yaml}).
            An config object describing what to call and what params to use.
    ...
    if config._target_ == "???":
        # Specific check to give a good warning about
        # failure to annotate _target_ as a string.
        raise ValueError(
            "Config has missing value for key `_target_`, cannot instantiate.\n"
            f"Config type: {type(config).__name__}\n"
            "Check that the `_target_` key in your dataclass is"
            " properly annotated and overridden.\n"
            "A common problem is forgetting to"
            " annotate _target_ as a string : '_target_: str = ...'"
        )
        # TODO: print full key
...
def instantiate_node(
    ...
    *args: Any,
    ...
    if OmegaConf.is_dict(node):
        # using getitem instead of get(key, default) because OmegaConf will
        # raise an exception if the key type is incompatible on get.
    ...
    if OmegaConf.is_list(node):
        ...
        if _is_target(node):
            ...
            # If ALL or PARTIAL non structured,
